from config import (chemical_elements, edges, atomic_nr, max_molecule_size)
from torch_geometric.utils import to_dense_adj
import torch
from config import dvc as device
...
def slice_atom_type_from_node_feats(node_features, as_index=False):
    ...
    """
    supported_atoms = chemical_elements
    atomic_numbers = atomic_nr
...
def triu_to_3d_dense(triu_values, num_nodes, depth=len(edges)):
    """
...
def calculate_node_edge_pair_loss(node_tar, edge_tar, node_pred, edge_pred):
    ...
    node_edge_preds = torch.empty((max_molecule_size, len(chemical_elements), len(edges)), dtype=torch.float,
                                  device=device)
    ...
    node_edge_tar = torch.empty((node_tar.shape[0], len(chemical_elements), len(edges)), dtype=torch.float,
                                device=device)
    for edge in range(len(edges)):
    ...
    node_edge_node_preds = torch.empty((max_molecule_size, max_molecule_size, len(edges)), dtype=torch.float,
                                       device=device)
    for edge in range(len(edges)):
        node_edge_node_preds[:, :, edge] = torch.matmul(node_edge_preds[:, :, edge], node_pred[:, :9].t())

    # Calculate node-edge-node for targets
    node_edge_node_tar = torch.empty((node_tar.shape[0], node_tar.shape[0], len(edges)), dtype=torch.float,
                                     device=device)
    for edge in range(len(edges)):
...
def approximate_recon_loss(node_targets, node_preds, triu_targets, triu_preds):
    ...
    onehot_node_targets = to_one_hot(node_targets, chemical_elements)  # + ["None"]
    onehot_triu_targets = to_one_hot(triu_targets, ["None"] + edges)

    # Reshape node predictions
    node_matrix_shape = (max_molecule_size, (len(chemical_elements) + 1))
    node_preds_matrix = node_preds.reshape(node_matrix_shape)

    # Reshape triu predictions
    edge_matrix_shape = (int((max_molecule_size * (max_molecule_size - 1)) / 2), len(edges) + 1)
...
def vae_loss(triu_logits, node_logits, edge_index, edge_types, node_types, \
    ...
    graph_size = max_molecule_size * (len(chemical_elements) + 1)
    graph_triu_size = int((max_molecule_size * (max_molecule_size - 1)) / 2) * (len(chemical_elements) + 1)
