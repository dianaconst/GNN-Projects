...
class VGAE(nn.Module):
    def __init__(self, feature_size):
        ...
        self.num_edge_types = len(edges)
        self.num_atom_types = len(chemical_elements)
        self.max_num_atoms = max_molecule_size
        self.decoder_hidden_neurons = 512

        # Encoder layers
        self.conv_1 = TransformerConv(feature_size,
                                     self.encoder_embedding_size,
                                     heads=4,
                                     concat=False,
                                     beta=True,
                                     edge_dim=self.edge_dim)
        self.norm_1 = BatchNorm(self.encoder_embedding_size)

        self.conv_2 = TransformerConv(self.encoder_embedding_size,
                                     self.encoder_embedding_size,
                                     heads=4,
                                     concat=False,
                                     beta=True,
                                     edge_dim=self.edge_dim)
        self.norm_2 = BatchNorm(self.encoder_embedding_size)

        self.conv_3 = TransformerConv(self.encoder_embedding_size,
                                     self.encoder_embedding_size,
                                     heads=4,
                                     concat=False,
                                     beta=True,
                                     edge_dim=self.edge_dim)
        self.norm_3 = BatchNorm(self.encoder_embedding_size)

        self.conv_4 = TransformerConv(self.encoder_embedding_size,
                                     self.encoder_embedding_size,
                                     heads=4,
                                     concat=False,
                                     beta=True,
    ...
    def sample_mols(self, num=10000):
        ...
        for _ in tqdm(range(num)):
            ...
            # Get atomic numbers
            node_preds_one_hot = to_one_hot(node_preds, options=atomic_nr)
            atom_numbers_dummy = torch.Tensor(atomic_nr).repeat(node_preds_one_hot.shape[0], 1)
